...
# from src import data, dist_utils, utils
...
def load_setting(opt):

    if opt.setting == 'ict':
        from ict import data
    if opt.setting == 'ind_cropping':
        from ind_cropping import data

def train(opt, model, optimizer, scheduler, step):

    logger.info("Data loading: load preprocessed/tokenized corpus")
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
        tokenizer = model.module.tokenizer
    else:
        tokenizer = model.tokenizer
    train_dataset = data.load_dataset(opt, tokenizer)
